...
<aside
	use:clickoutside
	on:clickoutside={() => isOpenSidebar.set(false)}
...
>
	<div class="py-6 relative">
		<button
			...
			class=" block md:hidden text-white p-1 text-xl absolute right-0 rounded-tl-md rounded-bl-md"
		>
			...
			{#each links as link}
				<li>
					{#if link.path}
						...
						>
							<Icon
								icon={link.path == routeId ? link.icon.active : link.icon.inactive}
								class="text-3xl mr-3"
							/>
	...
	<footer class="px-6 text-xs absolute bottom-6 text-gray-700">&copy; 2023 By Asher Azriel Ginting</footer>
</aside>
